...
/* 存档兼容：若玩家存档指向已删除的车，回退到 coupe。
   白名单与 content/vehicles.js 的 VEHICLES 保持一致。
   （core.js 位于依赖最底层，不能 import vehicles.js，故此处硬编码。） */
{
  const _VALID_CAR_IDS = ['coupe', 'motorcycle', 'siege'];
  if (!_VALID_CAR_IDS.includes(selectedCarId)) {
    selectedCarId = 'coupe';
    save('myCarSelectedCar', 'coupe');
  }
}
